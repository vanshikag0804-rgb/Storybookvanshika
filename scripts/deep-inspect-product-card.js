...
function dumpFull(node, depth = 0) {
  ...
  const sizing = `(alignSelf:${node.layoutAlign||'inherit'}, grow:${node.layoutGrow||0})`;

  console.log(`${indent}- [${node.type}] "${node.name}" (${node.id}) ${bbox} ${flex} ${font}${txt}${r}${radii}${fill}${stroke} ${sizing}`);
...
['106:9228', '106:9203', '106:9178', '106:9267'].forEach(id => {
  const n = findNode(figma.document, id);
  if (n) {
    console.log(`\n=================== COMPONENT ID: ${id} (${n.name}) ===================`);
    dumpFull(n);
  }
});
