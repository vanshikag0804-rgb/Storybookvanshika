...
const match = specs.filter(s => s.name.toLowerCase().includes('progress bar') || s.layerName?.toLowerCase().includes('progress bar'));
console.log('--- Specs ---');
console.log(JSON.stringify(match, null, 2));

function findNode(node, nameOrId) {
  if (node.id === nameOrId || node.name.toLowerCase() === nameOrId.toLowerCase()) return node;
  if (node.children) {
    for (const child of node.children) {
      const res = findNode(child, nameOrId);
      if (res) return res;
    }
  }
  return null;
